...
class AuditState extends Model
{
    ...
    public static function getFinishedStateId() {
        return self::getStateId('finished');
    }

    private static function getStateId($statusName) {
        $auditState = AuditState::where('name', '=', $statusName)->first();

        if(!is_null($auditState)) {
