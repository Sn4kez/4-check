...
class ListScoreNotificationTest extends TestCase
{
    ...
    public function testInvalidAccess($userKey, $scoreKey, $statusCode)
    {
        ...
        if ($scoreKey === 'score') {
            $uri = '/scores/' . $this->score->id . '/notice/' . $this->checklist->id;
        } else {
            $uri = '/scores/' . Uuid::uuid4()->toString() . '/notice/' . $this->checklist->id;
        }
        $this->json('GET', $uri);
